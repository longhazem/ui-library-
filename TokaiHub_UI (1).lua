...
-- ═══════════════════════════════════════════════
--  GITHUB ASSET LOADER
--  Tải ảnh/âm thanh từ GitHub → lưu vào Delta Workspace
...
-- ══ CACHE SYSTEM ══
-- Dùng "cache" folder thay vì Workspace
-- Cache folder tồn tại bền vững, không bị xóa khi clear Workspace
-- Đường dẫn: Delta/cache/TokaiHub/...
...
local function CheckVersion()
    ...
    if not ok or ver ~= VERSION then
        ...
        return false  -- cần tải lại
    end
    return true  -- cache còn dùng được
end
local cacheValid = CheckVersion()

local ASSET_MAP = {
    icon       = { path = CACHE_DIR.."/icon.png",
                   url  = REPO.."models/icon.png"                                          },
...
function Library:CreateWindow()
    ...
    -- Dùng icon từ GitHub nếu có, fallback về rbxthumb
    task.defer(function()
        task.wait(0.5)
    ...
    -- Lock button dùng ảnh từ GitHub
    local lockBtn = Instance.new("TextButton", toolbar)
    lockBtn.Size = UDim2.new(0,22,0,22)
    lockBtn.BackgroundColor3 = Color3.fromRGB(80,200,80)
    lockBtn.Text = ""
    Instance.new("UICorner", lockBtn).CornerRadius = UDim.new(0,5)
    local lockBtnStroke = Instance.new("UIStroke", lockBtn)
    lockBtnStroke.Color = Color3.new(1,1,1); lockBtnStroke.Thickness = 1.5; lockBtnStroke.Transparency = 0.8

    local lockImg = Instance.new("ImageLabel", lockBtn)
    lockImg.Size = UDim2.new(0,15,0,15)
    lockImg.AnchorPoint = Vector2.new(0.5,0.5)
    lockImg.Position = UDim2.new(0.5,0,0.5,0)
    ...
    task.spawn(function()
        for i = 1, 20 do
            local img = GetAsset("unlockIcon", "")
            if img ~= "" then lockImg.Image = img; break end
            task.wait(0.3)
        end
    end)

    -- Hover + press animation giống MakeToolBtn
    lockBtn.MouseEnter:Connect(function()
        TweenService:Create(lockBtn, TweenInfo.new(0.15,Enum.EasingStyle.Back,Enum.EasingDirection.Out), {Size=UDim2.new(0,25,0,25)}):Play()
        TweenService:Create(lockBtnStroke, TweenInfo.new(0.15), {Transparency=0.2}):Play()
    end)
    lockBtn.MouseLeave:Connect(function()
        TweenService:Create(lockBtn, TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.Out), {Size=UDim2.new(0,22,0,22)}):Play()
        TweenService:Create(lockBtnStroke, TweenInfo.new(0.15), {Transparency=0.8}):Play()
    end)
    lockBtn.MouseButton1Down:Connect(function()
        TweenService:Create(lockBtn, TweenInfo.new(0.07), {Size=UDim2.new(0,19,0,19)}):Play()
    end)
    lockBtn.MouseButton1Up:Connect(function()
        TweenService:Create(lockBtn, TweenInfo.new(0.2,Enum.EasingStyle.Back,Enum.EasingDirection.Out), {Size=UDim2.new(0,22,0,22)}):Play()
    end)
    ...
    lockBtn.MouseButton1Click:Connect(function()
        ...
        -- Đổi ảnh lock/unlock
        task.spawn(function()
            local key = isLocked and "lockIcon" or "unlockIcon"
            for i = 1, 20 do
    ...
    -- ══ BORDER RUNNER ══
    -- 1 Frame viền, UIStroke + UIGradient Color xoay
    -- Gradient: 2 vệt trắng ngắn đối xứng (trái/phải), nền đen = ẩn
    do
        ...
        -- 2 vệt trắng ngắn đối xứng 180°
        -- Đen = ẩn, Trắng = vệt sáng
        ...
        -- Sync position + visible với main
        coroutine.wrap(function()
            while true do
        ...
        -- Xoay cùng chiều kim đồng hồ, 42°/s
        coroutine.wrap(function()
    ...
    --   DROPDOWN HELPER
    -- ══════════════════════════════════
    local activeDropdown = nil  -- chỉ 1 dropdown mở 1 lúc
    ...
    function Library:CreateTab(name, iconText)
        ...
        -- Nếu iconText là URL gambar → dùng ImageLabel, ngược lại dùng text
        ...
        -- ImageLabel cho icon ảnh (dùng cho Home tab)
        local tabImg = nil
